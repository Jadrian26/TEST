...
const HomePage: React.FC = () => {
  ...
  const storeDisplayName = "Uniformes y Bordados Escolares";
  ...
  const configuredStoreImageUrl = mediaItems.find(item => item.id === visitStoreSection_MainImageId)?.public_url;
  const wazeButtonIconSrc = mediaItems.find(item => item.id === visitStoreSection_WazeButtonIconId)?.public_url;
  const googleMapsButtonIconSrc = mediaItems.find(item => item.id === visitStoreSection_GoogleMapsButtonIconId)?.public_url;
  ...
  return (
    ...
    <div className="space-y-12 md:space-y-16">
      ...
      >
        ...
        {heroSlides.length === 0 ? (
          <div className="absolute inset-0 bg-brand-secondary flex flex-col items-center justify-center text-center p-4" aria-label="Carrusel no configurado">
            ...
            <p className="text-base sm:text-lg text-brand-gray-light">El administrador puede añadir una imagen para esta sección</p>
          </div>
        ) : (
          <>
            {heroSlides.map((slide, index) => (
              <div
                ...
                style={{ backgroundImage: `url('${slide.public_url}')` }} 
          ...
          <div className="lg:w-5/12 aspect-square rounded-md mb-6 lg:mb-0 overflow-hidden">
            {configuredStoreImageUrl ? (
                ...
                <div className="w-full h-full bg-brand-secondary flex flex-col items-center justify-center text-center p-4 rounded-md text-text-on-secondary-bg" aria-label="Imagen de tienda no configurada">
                    ...
                    <p className="text-sm md:text-base">El administrador puede añadir una imagen</p>
                </div>
            )}
          </div>
      ...
      {currentUser?.isAdmin && (<EditCarouselSlidesModal isOpen={isEditCarouselModalOpen} onClose={closeEditCarouselModal} />)}
      {currentUser?.isAdmin && (<EditSchoolCarouselSettingsModal isOpen={isEditSchoolCarouselSettingsModalOpen} onClose={closeEditSchoolCarouselSettingsModal} />)}
      {currentUser?.isAdmin && (<EditVisitStoreSectionModal isOpen={isEditVisitStoreSectionModalOpen} onClose={closeEditVisitStoreSectionModal} />)} 
    </div>
     <style>{`
