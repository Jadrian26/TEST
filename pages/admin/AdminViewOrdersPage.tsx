    ...
    allUsers.forEach(user => { 
      user.orders.forEach(order => {
        ...
        orders.push({
          ...
          userId: order.userId, 
          customerSchoolId: orderOwner?.schoolId, 
        });
      });
    });
      ...
      <div className="bg-brand-primary p-4 rounded-lg shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          ...
          <div>
            ...
            <select
              id="schoolFilter"
              value={filters.customerSchoolId || ''}
              ...
              <thead className="text-xs text-text-primary uppercase bg-brand-gray-light bg-opacity-50">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3">Cliente</th>
                   { (currentUser?.isAdmin || currentUser?.isSales) && <th scope="col" className="px-4 py-3">Cédula Cliente</th>}
                  <th scope="col" className="px-4 py-3">Colegio Cliente</th>
              ...
              <tbody className="divide-y divide-brand-gray-light">
                {filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-brand-gray-light hover:bg-opacity-30 transition-colors">
                    ...
                    {(currentUser?.isAdmin || currentUser?.isSales) && <td className="px-4 py-2 text-xs sm:text-sm">{order.customerIdCardForOrder || 'N/A'}</td>}
                    <td className="px-4 py-2 text-xs sm:text-sm">{getSchoolName(order.customerSchoolId)}</td>
