...
const AdminDashboardPage: React.FC = () => {
  ...
  const totalOrders = registeredUsers.reduce((sum, user) => sum + (user.orders?.length || 0), 0);

  if (isLoadingContent) {
    return <div className="text-center py-10 text-text-secondary">Cargando datos del panel...</div>;
  }
  
  const panelTitle = "Panel de U&B"; // Unificado

  return (
    <div className="space-y-8">
      <div className="pb-6 border-b border-brand-quaternary border-opacity-30">
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-brand-secondary mb-1">{panelTitle}</h1>
        <p className="text-text-secondary text-base sm:text-lg">¡Bienvenido de nuevo, {currentUser?.firstName || 'Usuario'}! Resumen de tu aplicación.</p>
      </div>
