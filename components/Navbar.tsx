...
const Navbar: React.FC = () => {
  ...
  const showAdminPanelLink = currentUser?.role === 'admin' || currentUser?.role === 'sales';
  const adminPanelText = "U&B Panel"; // Unificado para ambos roles
  const canEditBrandLogo = currentUser?.role === 'admin' || currentUser?.role === 'sales';
            ...
            <div className="flex items-center space-x-1 sm:space-x-2">
              {(currentUser?.role === 'admin' || currentUser?.role === 'sales') && (
                <span 
                  ...
                  title={currentUser.role === 'admin' ? "Modo Administrador Activo" : "Modo Ventas Activo"}
                >
                  {currentUser.role === 'admin' ? "MODO ADMIN" : "MODO VENTAS"}
                </span>
              )}
              <Link 
                to="/account" 
          ...
          <nav className="flex flex-col space-y-1 px-4 pt-3">
            {(currentUser?.role === 'admin' || currentUser?.role === 'sales') && (
                <div className={`px-3 py-2 text-sm font-bold rounded-md text-center mb-2 ${
                  ...
                  {currentUser.role === 'admin' ? "MODO ADMIN ACTIVO" : "MODO VENTAS ACTIVO"}
                </div>
              )}
            {NAV_LINKS_PRIMARY.map((link) => (
              <NavLink
