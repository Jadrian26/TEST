...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ onLinkClick }) => {
  ...
  // Links only Admins see
  const adminExclusiveLinks = [
    ...
    // The /admin/pdf-config link was previously here and is now removed from the sidebar.
  ];
  ...
  if (currentUser?.isAdmin) {
    ...
    // Order: Dashboard, AdminExclusives, then rest of common (Users, Orders)
    linksToShow = [
      dashboardLink,
      ...adminExclusiveLinks,
    ...
    linksToShow = baseCommonLinks; // Sales see only common links
  }


  const panelTitle = currentUser?.isAdmin ? "PANEL ADMIN" : (currentUser?.isSales ? "PANEL VENTAS" : "PANEL");
      ...
      <nav className="flex-1 px-2 sm:px-3 py-3 sm:py-4 space-y-1 sm:space-y-1.5 overflow-y-auto">
        {linksToShow.map(linkInfo => (
          <React.Fragment key={linkInfo.to}>
            ...
            {/* The conditional NavLink for /admin/pdf-config has been removed from here */}
          </React.Fragment>
        ))}
      </nav>
