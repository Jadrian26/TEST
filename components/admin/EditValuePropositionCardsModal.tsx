...
const EditValuePropositionCardsModal: React.FC<EditValuePropositionCardsModalProps> = ({ isOpen, onClose }) => {
  const { valuePropositionCardsData, updateValuePropositionCardsData } = useEditableContent();
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    try {
      updateValuePropositionCardsData(cardsToEdit);
          ...
          <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto pr-1 space-y-6">
            {cardsToEdit.map((card, index) => {
              ...
              if (selectedMediaItem) {
                iconPreviewElement = <img src={selectedMediaItem.dataUrl} alt={card.title} className="w-10 h-10 object-contain" />;
              } else {
              ...
              return (
                <fieldset key={card.id} className="border border-brand-quaternary p-4 rounded-md">
