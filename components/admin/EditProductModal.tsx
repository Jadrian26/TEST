...
const EditProductModal: React.FC<EditProductModalProps> = ({ isOpen, onClose, productToEdit }) => {
  ...
  const [currentProductSchoolId, setCurrentProductSchoolId] = useState<string>(''); 
  ...
  const form = useFormHandler({
    ...
    onSubmit: async (values) => {
      ...
      try {
        updateProductContext(productToEdit.id, {
          name: values.name,
  ...
  const handleSelectImage = (selectedMedia: MediaItem) => {
    form.setValues(prev => ({ ...prev, imageUrl: selectedMedia.dataUrl }));
    setImagePreview(selectedMedia.dataUrl);
  ...
  const schoolName = productToEdit?.schoolId ? schools.find(s => s.id === productToEdit.schoolId)?.name : 'Producto Genérico (Sin Colegio Asociado)';
